...
1. **Docker Architecture**:
   - เหตุใดจึงต้องแยก database และ application เป็นคนละ containers ?
     เพื่อแยกหน้าที่ (Separation of Concerns) ให้แต่ละส่วนทำงานอิสระต่อกัน ช่วยให้ระบบยืดหยุ่น ปลอดภัย ดูแลและปรับปรุงง่าย รวมถึงสามารถ scale หรืออัปเดตแยกส่วนได้โดยไม่กระทบกัน
   - Multi-stage build มีประโยชน์อย่างไร?
     ช่วยลดขนาดของ Docker image, เพิ่มความปลอดภัย, ลด dependency ที่ไม่จำเป็น และทำให้ build เร็วขึ้นโดยใช้หลายขั้นตอนแยกกัน (build → copy → run)

2. **Testing Strategy**:
   - การวัด code coverage มีความสำคัญอย่างไร?
     ช่วยตรวจสอบว่าการทดสอบครอบคลุมโค้ดมากน้อยเพียงใด เป็นตัวชี้วัดคุณภาพของ test case ป้องกัน regression bug และเพิ่มความมั่นใจในการ deploy โค้ดเข้าสู่ production

3. **Deployment**:
   - Health check endpoint มีความสำคัญอย่างไร?
     ใช้ตรวจสอบว่าสถานะของแอปยังทำงานปกติหรือไม่ ช่วยให้ระบบอัตโนมัติ เช่น load balancer หรือ orchestrator สามารถ restart หรือหยุด instance ที่ไม่พร้อมได้
   - Render และ Railway มีความแตกต่างกันอย่่างไร?
     Render: เหมาะกับ production ใช้งานจริง เสถียร ปรับแต่งได้มาก
     Railway: เหมาะกับการทดลองหรือโปรเจกต์เล็ก ใช้งานง่าย ตั้งค่าอัตโนมัติ แต่มีข้อจำกัดด้านเวลาและทรัพยากรใน free plan
